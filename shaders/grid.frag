...
void main()
{
    ...
    float grid_res = dot(dFdx(frag_pos), inverse(u_View)[0].xyz);
    grid_res *= 4;
...
//    blend = blend * blend * blend;
    ...
    vec2 grid_pos = frag_pos.xz;

    float gridA = get_grid(grid_pos, derivative, vec2(scale_b, scale_b));
    float gridB = get_grid(grid_pos, derivative, vec2(scale_c, scale_c));
    float gridC = get_grid(grid_pos, derivative, vec2(scale_d, scale_d));

    vec4 color_grid = vec4(0.2, 0.2, 0.2, 1.0);
    vec4 color_grid_emphasis = vec4(0.0, 1.0, 0.0, 1.0);
